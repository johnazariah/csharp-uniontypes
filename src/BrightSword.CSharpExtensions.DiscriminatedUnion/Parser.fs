...
module internal Parser = 
    ...
    let wchar : char -> Parser<char, unit> = pchar >> ws >> attempt
    ...
    let identifier = spaces >>. regex "[\p{Ll}\p{Lu}\p{Lt}\p{Lo}\p{Lm}_][\p{Ll}\p{Lu}\p{Lt}\p{Lo}\p{Lm}\d_]*[?]?" .>> spaces
    ...
    do fullTypeNameImpl := spaces >>. identifier .>>. many dotComponent .>>. opt typeArguments |>> FullTypeName.apply

    let memberName = word |>> UnionMemberName
    let caseClassMember = 
        (wstr "case class" >>. memberName) .>>. (pointed fullTypeName) .>> wstr ";" 
        |>> UnionMember.CaseClass <?> "Case Class"
    let caseObjectMember = 
        wstr "case object" >>. memberName .>> wstr ";" 
        |>> UnionMember.CaseObject <?> "Case Object"
    ...
    let unionType = 
        ...
        .>>. caseMembers 
        |>> (UnionType.apply >> UnionType) <?> "Union"
